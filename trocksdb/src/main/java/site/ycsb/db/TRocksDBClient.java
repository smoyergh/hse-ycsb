 ...
 */
public class TRocksDBClient extends DB {
  ...
  private static final Logger LOGGER = LoggerFactory.getLogger(TRocksDBClient.class);

  @GuardedBy("TRocksDBClient.class") private static Path rocksDbDir = null;
  @GuardedBy("TRocksDBClient.class") private static Path optionsFile = null;
  @GuardedBy("TRocksDBClient.class") private static RocksObject dbOptions = null;
  @GuardedBy("TRocksDBClient.class") private static RocksDB rocksDb = null;
  @GuardedBy("TRocksDBClient.class") private static int references = 0;
  ...
  public void init() throws DBException {
    synchronized(TRocksDBClient.class) {
      if(rocksDb == null) {
        rocksDbDir = Paths.get(getProperties().getProperty(PROPERTY_ROCKSDB_DIR));
        LOGGER.info("TRocksDB data dir: " + rocksDbDir);
        ...
        if (optionsFileString != null) {
          ...
          LOGGER.info("TRocksDB options file: " + optionsFile);
        }
   ...
   * Initializes and opens the RocksDB database.
   *
   * Should only be called with a {@code synchronized(TRocksDBClient.class)` block}.
   *
   * @return The initialized and open RocksDB instance.
   */
      ...
      if(!COLUMN_FAMILIES.containsKey(name)) {
        ...
        if (optionsFile != null) {
          if (COLUMN_FAMILIES.containsKey("default")) {
            LOGGER.warn("no column family options for \"" + name + "\" " +
                        "in options file - using options from \"default\"");
            cfOptions = COLUMN_FAMILIES.get("default").getOptions();
          } else {
            LOGGER.warn("no column family options for either \"" + name + "\" or " +
                        "\"default\" in options file - initializing with empty configuration");
            cfOptions = new ColumnFamilyOptions();
          }
          LOGGER.warn("Add a CFOptions section for \"" + name + "\" to the options file " +
